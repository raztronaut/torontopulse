...
import { BikeShareTorontoPlugin } from './index.js';

describe('BikeShareTorontoPlugin', () => {
  let plugin: BikeShareTorontoPlugin;

  beforeEach(() => {
    plugin = new BikeShareTorontoPlugin();
  });

  describe('Plugin Metadata', () => {
    it('should have correct metadata', () => {
      expect(plugin.metadata.id).toBe('bike-share-toronto');
      expect(plugin.metadata.name).toBe('Bike Share Toronto');
