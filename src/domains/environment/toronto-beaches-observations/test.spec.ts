...
import { TorontoBeachesObservationsPlugin } from './index.js';

describe('TorontoBeachesObservationsPlugin', () => {
  let plugin: TorontoBeachesObservationsPlugin;

  beforeEach(() => {
    plugin = new TorontoBeachesObservationsPlugin();
  });

  describe('Plugin Metadata', () => {
    it('should have correct metadata', () => {
      expect(plugin.metadata.id).toBe('toronto-beaches-observations');
      expect(plugin.metadata.name).toBe('Toronto Beaches Observations');
      expect(plugin.metadata.domain).toBe('environment');
