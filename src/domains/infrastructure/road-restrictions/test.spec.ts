...
import { RoadRestrictionsPlugin } from './index.js';

describe('RoadRestrictionsPlugin', () => {
  let plugin: RoadRestrictionsPlugin;

  beforeEach(() => {
    plugin = new RoadRestrictionsPlugin();
  });

  describe('Plugin Metadata', () => {
    it('should have correct metadata', () => {
      expect(plugin.metadata.id).toBe('road-restrictions');
      expect(plugin.metadata.name).toBe('Road Restrictions');
